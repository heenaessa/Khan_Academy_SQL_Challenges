...
/*What are average, max, and min values in the data? */
SELECT MAX(worldwide), MAX(domestic), MAX(overseas)
FROM topmovies;

SELECT COUNT(*) FROM topmovies;

/*To find the average, maximum, and minimum values of the Worldwide column in the topmovies table*/
SELECT AVG(Worldwide) AS Average, MAX(Worldwide) AS Maximum, MIN(Worldwide) AS Minimum
FROM topmovies;

/*To find the average, maximum, and minimum values of the Worldwide column in the topmovies table per year*/
SELECT Year, AVG(Worldwide) AS Average, MAX(Worldwide) AS Maximum, MIN(Worldwide) AS Minimum
FROM topmovies
GROUP BY Year;

/*To find the average, maximum, and minimum values of the Worldwide column in the topmovies table per studio*/
SELECT Studio, AVG(Worldwide) AS Average, MAX(Worldwide) AS Maximum, MIN(Worldwide) AS Minimum
FROM topmovies
GROUP BY Studio;

/*To group the data values that don’t exist yet using CASE*/
SELECT Title, Year,
       CASE
           WHEN Worldwide >= 2000 THEN 'Blockbuster'
           WHEN Worldwide >= 1000 THEN 'Hit'
           ELSE 'Flop'
       END AS Category
FROM topmovies;
/*To filter the data in interesting ways using AND/OR*/
SELECT *
FROM topmovies
WHERE (Studio = 'BV' OR Studio = 'Fox') AND Year >= 2010 AND Worldwide >= 1500;

/*SQL query that returns the rank, title, and worldwide gross of the top 6 highest-grossing movies of all time */
SELECT Rank, Title, Worldwide
FROM topmovies
ORDER BY Rank ASC;

/*To find the total worldwide gross of all movies released in odd years*/
SELECT SUM(Worldwide) AS Total
FROM topmovies
WHERE Year % 2 = 1;

/*To find the average and maximum values of the DomesticPct column in the topmovies table per year, for movies with a worldwide gross of 1500 or more*/
SELECT Year, AVG(DomesticPct) AS Average, MAX(DomesticPct) AS Maximum
FROM topmovies
WHERE Worldwide >= 1500
GROUP BY Year;
